...
class HBNBCommand(cmd.Cmd):
    ...
    def do_create(self, line):
        ...
        args = line.split()
        # models to dynamically create base on class name.
        ...
        model = self.ALL_CLASSES[cls_name]

        instance: BaseModel = model()
        print(instance.id)
        instance.save()


        # iterate through the args list from index 1 since we know index 0 is the class name to create.
        for i in range(1, len(args)):
            if "=" in args[i]:
                attr, value = args[i].split("=")
    ...
    def do_all(self, arg):
        ...
        if arg:
            if arg in self.ALL_CLASSES:
                for k, v in storage.all(self.ALL_CLASSES[arg]).items():
